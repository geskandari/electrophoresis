...
class DatabaseConnection:
    ...
    def __init__(self):
        db_info = pd.read_excel('DB_info.xlsx')
        DB_hostname = db_info['DB_hostname'].iloc[0]
        DB_name = db_info['DB_name'].iloc[0]
        DB_username = db_info['DB_username'].iloc[0]
        DB_password = db_info['DB_password'].iloc[0]
    ...
    def get_rows_for_date(self, dateObject):
        ...
        cur.execute("""select data_analisi, data_prel, seq, programma, eta, pt,
            ...
            from anagrafica where programma = 'S' AND data_analisi = (%s)""", [dateObject])
    ...
    def get_history(self, mrn):
        ...
        cur_history.execute("""select data_analisi, data_prel, seq, programma, commento1, val_picco_1, free3, free5, longmemo, free1
                    from anagrafica where free1 = (%s)""", [mrn])
...
class GUI:
    def __init__(self, database_connection):
        ...
        self.table.model.columnlabels['1'] = "CollectionDate"
        ...
        self.history_comment.place(relx=0.1, rely=0.2, relwidth=0.8, relheight=0.6)

        self.attending = tk.Label(self.hframe, text='', font=15)
        self.attending.place(relx=0.125, rely=0.8, relwidth=0.8, relheight=0.2)
    ...
    def clicked(self, event):  # Click event callback function.
        try:
            ...
            self.attending.config(text=currentPatientHistoryRecord[4])

            self.history_comment.delete('1.0', tk.END)
            self.history_comment.insert(tk.END, str(currentPatientHistoryRecord[8]))
    ...
    def getCurrentPatientHistoryRecord(self):
        ...
        x = 0
        while x < len(self.patient_history):
            if currentTableRecord['1'] == self.patient_history[x][1].strftime("%Y-%m-%d") and (
                    (currentTableRecord['2'] == 'SPEP' and self.patient_history[x][3] == 'S') or (
                    currentTableRecord['2'] == 'UPEP' and self.patient_history[x][3] == '6') or (
                            currentTableRecord['2'] == 'OCBE' and self.patient_history[x][3] == 'A') or (
                            currentTableRecord['2'] == 'CSPE' and self.patient_history[x][3] == 'J')) and \
                    currentTableRecord['3'] == self.patient_history[x][6] and currentTableRecord['4'] == \
                    self.patient_history[x][5]:
                thisPatientHistory = self.patient_history[x]
    ...
    def updateTK(self):
        ...
        self.History.config(text='Case ' + str(
            self.rowNumber + 1) + ': Does ' + self.thisRowData.patientName + ' have a previous history?')
        ...
        try:
            ...
            print("error getting patient history")
            print(str(e))
            return

        # add a row for each new history element
        n = 0
        while n < len(self.patient_history):
            new_row_index = self.table.model.addRow() - 1

            if self.patient_history[n][1] is not None:
                self.table.model.setValueAt(self.patient_history[n][1].strftime("%Y-%m-%d"), new_row_index, 0)
            if self.patient_history[n][3] == 'S':
                self.table.model.setValueAt('SPEP', new_row_index, 1)
            elif self.patient_history[n][3] == 'J':
                self.table.model.setValueAt('CSPE', new_row_index, 1)
            elif self.patient_history[n][3] == 'A':
                self.table.model.setValueAt('OCBE', new_row_index, 1)
            elif self.patient_history[n][3] == '6':
                self.table.model.setValueAt('UPEP', new_row_index, 1)
            self.table.model.setValueAt(self.patient_history[n][6], new_row_index, 2)
            self.table.model.setValueAt(self.patient_history[n][5], new_row_index, 3)
            n = n + 1
